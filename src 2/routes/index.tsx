...
function Index() {
  ...
  const lapsedFiltered = useMemo(() => {
    ...
    // Sort by oldest purchase first (most overdue at the top)
    return [...list].sort((a, b) => {
      const da = a.purchase_date ? new Date(a.purchase_date).getTime() : 0;
      const db = b.purchase_date ? new Date(b.purchase_date).getTime() : 0;
      return da - db;
    });
  }, [lapsed, lapsedYear, lapsedMonth, lapsedHideContacted]);
...
function LapsedTab({
  ...
  yearOptions, monthOptions, hideContacted, setHideContacted,
}: {
  ...
  yearOptions: number[]; monthOptions: { value: string; label: string }[];
    ...
    <>
      <header className="bg-white px-5 pt-8 pb-6 rounded-b-[2rem] shadow-sm">
        ...
        <p className="text-[11px] mt-3" style={{ color: "var(--slate-500)" }}>
          הסינון לפי חודש/שנה של הרכישה האחרונה. סמן ✓ אחרי שדיברת עם הלקוח.
        </p>
      </header>
      <main className="px-5 py-6">
        {list.length === 0
